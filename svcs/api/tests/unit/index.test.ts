...
test("createNewURouter returns health without requiring a request body", async () => {
...
test("createNewURouter returns alive and ready without auth", async () => {
  const router = createNewURouter();
...
test("createNewUApp creates a Koa request handler", () => {
  const app = createNewUApp();

  assert.equal(typeof app.callback(), "function");
});

test("createNewURouter routes Better Auth endpoints through the auth handler", async () => {
  let routedPath = "";
  const router = createNewURouter({
  ...
  const response = await router(new Request("http://localhost/api/auth/session"));

  assert.strictEqual(response.status, 204);
  assert.strictEqual(routedPath, "/api/auth/session");
});

test("createNewURouter protects package routes with Better Auth", async () => {
  const router = createNewURouter();
  const response = await router(
...
test("authorizeBetterAuthRequest verifies x-api-key after a missing session", async () => {
  let verifiedKey = "";
  const authApi = createApiKeyAuthApi((key) => {
    verifiedKey = key;
  });
  const request = new Request("http://localhost/api/v1/packages", {
    headers: { "x-api-key": "key_123" },
  });
  const user = await authorizeBetterAuthRequest(request, authApi);
  ...
  assert.strictEqual(verifiedKey, "key_123");
});

test("authorizeBetterAuthRequest verifies bearer API keys after a missing session", async () => {
  let verifiedKey = "";
  const authApi = createApiKeyAuthApi((key) => {
    verifiedKey = key;
  }, { name: null, referenceId: "43" });
  const request = new Request("http://localhost/api/v1/packages", {
    headers: { authorization: "Bearer key_456" },
  });
  const user = await authorizeBetterAuthRequest(request, authApi);
  ...
  assert.strictEqual(verifiedKey, "key_456");
});

test("createNewURouter returns Problem Details for malformed package paths", async () => {
  const router = createNewURouter();
  const response = await router(
    new Request("http://localhost/api/v1/packages/util/arr/%E0%A4%A"),
  );
  const body = await response.json();

  assert.strictEqual(response.status, 400);
  assert.strictEqual(body.title, "invalid-package-path");
  assert.match(
...
test("createNewURouter rate limits protected routes by authenticated user", async () => {
...
test("createNewURouter caches repeated GET package reads", async () => {
  const router = createAuthedRouter({
    cache: {
      now: () => 1_000,
    },
...
test("createNewURouter resolves packages by query", async () => {
  const router = createAuthedRouter();
  const response = await router(
    authedRequest("http://localhost/api/v1/packages?kind=util&q=head"),
  );
...
test("createNewURouter returns package detail from the registry", async () => {
  const router = createAuthedRouter();
  const response = await router(
  ...
  assert.strictEqual(body.data.tags.includes("array"), true);
...
test("createNewURouter returns service package detail from the registry", async () => {
  const router = createAuthedRouter();
  const response = await router(
...
test("createNewURouter returns package representation branches", async () => {
  const router = createAuthedRouter();
  ...
  const installResponse = await router(authedRequest(installUrl));
...
test("createNewURouter returns tags, patterns, usage, setup, and tests branches", async () => {
  const router = createAuthedRouter();
  ...
  const setupResponse = await router(authedRequest(setupUrl));
...
test("createNewURouter returns code blocks by numeric id or key", async () => {
  const router = createAuthedRouter();
  const numericUrl = "http://localhost/api/v1/packages/util/arr/head/code-blocks/0";
  const numericResponse = await router(authedRequest(numericUrl));
...
test("createNewURouter returns a preview resource for a POST", async () => {
  const router = createAuthedRouter();
  const response = await router(
...
test("createNewURouter writes only when the REST request opts in", async () => {
...
test("createNewURouter returns Problem Details for unsupported request fields", async () => {
  const router = createAuthedRouter();
  const response = await router(
    authedRequest("http://localhost/api/v1/packages", {
...
function createApiKeyAuthApi(
  onVerify: (key: string) => void,
  key: ApiKeyRecord = { name: "api-ci", referenceId: "42" },
): BetterAuthApi {
  return {
    getSession: async () => null,
    verifyApiKey: async (context) => {
      onVerify(context.body.key);
